...
def main():
    ...
    vy_value_to_test = np.linspace(vy_start, vy_end, num_points) 
    ...
    if not results_mass_np.size:
        ...
        title = (f"Vy[{vy_start:.2e}-{vy_start:.2e}]_M[{mass_start:.2e}-{mass_start:.2e}]_num[{num_points}]_mat[{material_name}]")
        ...
        threeDPlotter(results_mass_np,results_v_y_np,results_temp_change, "Mass (kg)", "Velocity (m/s)", "Temparature Change (K)") 
        #added more params for threeDplotter, they are axis labels - EN
